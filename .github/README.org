...
#+begin_src bash
nix --experimental-features "nix-command flakes" run 'github:nix-community/disko#disko-install' -- --write-efi-boot-entries --flake 'github:lazycaat/nixos-config/develop#machine1' --disk main /dev/vda
#+end_src
